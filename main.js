/* =========================
   Son of Wisdom — main.js  (Call + Chat + Live AI captions + Barge-in)
   - Unlimited user recording (ends after ~3s silence)
   - AI audio plays fully, no timeout caps
   - Barge-in: user speech during AI reply interrupts playback and re-opens listening
   - Chat bubbles; AI bubble can live-type via optional SSE/JSON transcript webhook
   - n8n voice path must respond with BINARY audio (audio/mpeg|webm)
   - Optional AI-transcript webhook (SSE or JSON) for live text: N8N_TRANSCRIBE_URL
   ========================= */
...
/* Optional: AI-transcript webhook (leave empty to disable).
   Supports SSE (preferred) or JSON response with {text} */
...
function getOrCreateDeviceId(){
  ...
  if (!id) { id = crypto.randomUUID?.() || `anon_${Date.now()}_${Math.random().toString(36).slice(2)}`; localStorage.setItem(DEVICE_ID_KEY, id); }
  return id;
}
function getUserIdForWebhook(){ return localStorage.getItem(USER_ID_KEY) || getOrCreateDeviceId(); }
const pickUuidForHistory = (user_id)=>(USER_UUID_OVERRIDE && isUuid(USER_UUID_OVERRIDE)) ? USER_UUID_OVERRIDE : (isUuid(user_id)?user_id:SENTINEL_UUID);
...
const HAS_NATIVE_ASR = ('SpeechRecognition' in window) || ('webkitSpeechRecognition' in window);
...
let micMuted=false, speakerMuted=false;
...
async function fetchLastPairsFromSupabase(user_id,{pairs=8}={}) {
  try{
    ...
    const lastPairs = rows.map(r=>({ user: trimText(r.input_transcript), assistant: trimText(r.ai_text) }));
    const textBlock = lastPairs.map(p=>{
  ...
  }catch{ return { text:"", pairs:[] }; }
}
const SUMMARY_TABLE="history_summaries", SUMMARY_MAX_CHARS=380;
...
async function upsertRollingSummary(user_id, device, summary){
  try{
    const uuid = pickUuidForHistory(user_id);
    const body=[{ user_id_uuid:uuid, device_id:device, summary, last_turn_at:new Date().toISOString() }];
...
function buildRollingSummary(prevSummary,pairs,newest,maxChars=SUMMARY_MAX_CHARS){
  ...
  for(const p of pairs.slice(-6)){ if(p.user) sentences.push(`User: ${p.user}`); if(p.assistant) sentences.push(`Assistant: ${p.assistant}`); }
  if(newest) sentences.push(`User now: ${newest}`);
  const scored = sentences.map(s=>{
    const t=s.trim().replace(/\s+/g," "); let score=0;
...
function ensureChatUI(){
  if (!chatPanel){
    ...
    const anchor=document.getElementById("transcript")||document.getElementById("avatar-container")||document.body;
...
function showChatView(){ inChatView=true; chatPanel.style.display="block"; statusText.textContent="Chat view on. Call continues in background."; updateModeBtnUI(); }
function showCallView(){ inChatView=false; chatPanel.style.display="none"; statusText.textContent=isCalling?"Call view on.":"Tap the blue call button to begin."; updateModeBtnUI(); }
...
async function typewriter(el, full, delay=24){
  if(!el) return;
  el.textContent="";
  for (let i=0;i<full.length;i++){ el.textContent += full[i]; await new Promise(r=>setTimeout(r,delay)); }
}

/* transcript list helpers (call view) */
let lastFinalLine="";
const transcriptUI={
  clearAll(){ transcriptInterim.textContent=""; transcriptList.innerHTML=""; lastFinalLine=""; },
  setInterim(t){ transcriptInterim.textContent=t||""; },
  addFinalLine(t){
    const s=(t||"").trim();
...
(function setupCanvas(){
  ...
  voiceRing.width=size*dpr; voiceRing.height=size*dpr;
  voiceRing.style.width=`${size}px`; voiceRing.style.height=`${size}px`;
  voiceRing.getContext("2d").scale(dpr,dpr);
  drawVoiceRing();
})();
function drawVoiceRing(th=9,color="#d4a373"){
  const ctx=voiceRing.getContext("2d");
  const dpr=window.devicePixelRatio||1, w=voiceRing.width/dpr, h=voiceRing.height/dpr;
  ctx.clearRect(0,0,w,h);
  ctx.beginPath(); ctx.arc(w/2,h/2,85,0,Math.PI*2); ctx.lineWidth=th;
  ctx.strokeStyle=color; ctx.shadowBlur=15; ctx.shadowColor=color+"99"; ctx.stroke();
}
let ringCtx=null, ringAnalyser=null, ringRAF=null;
function stopRing(){
  if(ringRAF) cancelAnimationFrame(ringRAF); ringRAF=null;
  try{ ringAnalyser?.disconnect(); }catch{}
  if (ringCtx && ringCtx.state!=="closed"){ try{ ringCtx.close(); }catch{} }
...
function animateRingFromElement(mediaEl,color="#d4a373"){
  ...
  let src=null, analyser=null, gain=null, rafId=null;
  const start=()=>{
    stop();
    src=playbackAC.createMediaElementSource(mediaEl);
    gain=playbackAC.createGain();
    analyser=playbackAC.createAnalyser(); analyser.fftSize=1024; analyser.smoothingTimeConstant=0.7;
    try{ src.connect(gain); gain.connect(analyser); analyser.connect(playbackAC.destination); }catch{ return; }
    const data=new Uint8Array(analyser.fftSize);
    const loop=()=>{
      analyser.getByteTimeDomainData(data);
      let sum=0; for(let i=0;i<data.length;i++){ const v=data[i]-128; sum+=v*v; }
      const rms=Math.sqrt(sum/data.length); const th=10+Math.min(rms*1.0,34);
  ...
  const stop=()=>{
    ...
    try{ analyser?.disconnect(); gain?.disconnect(); src?.disconnect(); }catch{}
    drawVoiceRing();
  };
...
const VAD={ SILENCE_THRESHOLD:5, SILENCE_TIMEOUT_MS:3000, GRACE_MS:900, MIN_RECORD_MS:700 };
let vadCtx=null, vadAnalyser=null, vadSource=null, vadRAF=null, silenceMs=0;
function startMicVAD(stream,color="#d4a373"){
  vadCtx=new (window.AudioContext||window.webkitAudioContext)(); if(vadCtx.state==="suspended") vadCtx.resume().catch(()=>{});
  vadSource=vadCtx.createMediaStreamSource(stream); vadAnalyser=vadCtx.createAnalyser();
  vadAnalyser.fftSize=2048; vadAnalyser.smoothingTimeConstant=0.75; vadSource.connect(vadAnalyser);
  ringCtx=vadCtx; ringAnalyser=vadAnalyser;
  const data=new Uint8Array(vadAnalyser.fftSize);
  const startedAt=performance.now(); let last=performance.now(); silenceMs=0;
  const animate=()=>{
    ...
    const graceOver = elapsed>VAD.GRACE_MS, minLen=elapsed>VAD.MIN_RECORD_MS;

    // ring from analyser
    let acc=0; for(let i=0;i<data.length;i++){ const v=data[i]-128; acc+=v*v; }
    const rms=Math.sqrt(acc/data.length); const th = 10+Math.min(rms*0.9,32);
    drawVoiceRing(th, color);

    if(graceOver){
      if(level < VAD.SILENCE_THRESHOLD){
        silenceMs += dt;
        if(silenceMs >= VAD.SILENCE_TIMEOUT_MS && minLen && mediaRecorder?.state==="recording"){
          mediaRecorder.stop();
        }
      } else { silenceMs = 0; }
    }
...
function stopMicVAD(){
  ...
  if (vadCtx && vadCtx.state!=="closed"){ try{ vadCtx.close(); }catch{} }
...
function registerAudioElement(a){
  ...
  a.muted=speakerMuted; a.volume=speakerMuted?0:1;
...
async function routeElementToPreferredOutput(el){
  ...
  try{ await el.setSinkId(preferredOutputDeviceId); }catch{}
}
...
function updateMicTracks(){ if(globalStream) globalStream.getAudioTracks().forEach(t=> t.enabled=!micMuted); }
function updateSpeakerUI(){ speakerBtn?.setAttribute("aria-pressed", String(!speakerMuted)); }
function updateMicUI(){ micBtn?.setAttribute("aria-pressed", String(micMuted)); }
function updateModeBtnUI(){ if(modeBtn) { modeBtn.setAttribute("aria-pressed", String(inChatView)); modeBtn.title=inChatView?"Switch to Call view":"Switch to Chat view"; }}

/* ---------- Controls ---------- */
callBtn?.addEventListener("click", ()=>{ !isCalling ? startCall() : endCall(); });
micBtn?.addEventListener("click", ()=>{ micMuted=!micMuted; updateMicTracks(); updateMicUI(); statusText.textContent = micMuted?"Mic muted.":"Mic unmuted."; });
speakerBtn?.addEventListener("click", async()=>{
  const wasMuted=speakerMuted; speakerMuted=!speakerMuted;
  for(const el of managedAudios){ el.muted=speakerMuted; el.volume=speakerMuted?0:1; }
  updateSpeakerUI();
  if(wasMuted && !speakerMuted && "setSinkId" in HTMLMediaElement.prototype){
    ...
    if(preferredOutputDeviceId){ for(const el of managedAudios) await routeElementToPreferredOutput(el); statusText.textContent="Speaker output active."; }
  }
});
modeBtn?.addEventListener("click", ()=>{ inChatView ? showCallView() : showChatView(); });
document.addEventListener("keydown",(e)=>{ if(e.key?.toLowerCase?.()==="c"){ inChatView ? showCallView() : showChatView(); }});

/* ---------- Call flow ---------- */
async function startCall(){
  if(isCalling) return;
  isCalling=true; callBtn.classList.add("call-active");
  transcriptUI.clearAll();
  showCallView();

  try{
    statusText.textContent="Ringing…";
    await safePlayOnce("ring.mp3", {limitMs:15000}); if(!isCalling) return;

    statusText.textContent="AI is greeting you…";
    await safePlayOnce("blake.mp3", {limitMs:15000}); if(!isCalling) return;
...
function endCall(){
  isCalling=false; isRecording=false; isPlayingAI=false;
  callBtn.classList.remove("call-active");
  statusText.textContent="Call ended.";
  stopMicVAD(); stopRing(); stopBargeInMonitor();
  ...
  for(const el of Array.from(managedAudios)){ try{ el.pause(); const src=el.src; el.src=""; if(src?.startsWith("blob:")) URL.revokeObjectURL(src); }catch{} managedAudios.delete(el); }
}
...
function safePlayOnce(src,{limitMs=15000,color="#d4a373"}={}){
  return new Promise(res=>{
    ...
    let done=false; const settle=()=>{ if(done) return; done=true; a.onended=a.onerror=a.onabort=a.oncanplaythrough=null; stopRing(); res(true); };
    ...
    a.onended=()=>{ clearTimeout(t); settle(); };
  });
}
...
/* ---------- One turn capture ---------- */
let interimBuffer="", finalSegments=[];
...
async function captureOneTurn(){
  ...
  finalSegments=[]; interimBuffer="";
  transcriptUI.setInterim(HAS_NATIVE_ASR?"Listening…":"Listening…");

  try{
    const stream = await navigator.mediaDevices.getUserMedia({ audio:{ echoCancellation:true, noiseSuppression:true, autoGainControl:true }});
    if(!isCalling){ stream.getTracks().forEach(t=>t.stop()); return false; }
    globalStream=stream;

    let opts={};
    if(MediaRecorder.isTypeSupported("audio/webm;codecs=opus")){
      opts.mimeType="audio/webm;codecs=opus"; if(ENABLE_MEDIARECORDER_64KBPS) opts.audioBitsPerSecond=64_000;
    }else if(MediaRecorder.isTypeSupported("audio/webm")){
      opts.mimeType="audio/webm"; if(ENABLE_MEDIARECORDER_64KBPS) opts.audioBitsPerSecond=64_000;
    }
    try{ mediaRecorder=new MediaRecorder(stream,opts); }catch{ mediaRecorder=new MediaRecorder(stream); }
    recordChunks=[]; isRecording=true;

    mediaRecorder.ondataavailable=(e)=>{ if(e.data?.size>0){ recordChunks.push(e.data); HumeRealtime?.handleRecorderChunk?.(e.data); } };
    mediaRecorder.onstop=()=>{ commitInterimToFinal(); isRecording=false; stopMicVAD(); stopRing(); transcriptUI.setInterim(""); closeNativeRecognizer(); HumeRealtime?.stopTurn?.(); };

    startMicVAD(stream,"#d4a373"); openNativeRecognizer(); HumeRealtime?.startTurn?.(stream, vadCtx);
    mediaRecorder.start(TIMESLICE_MS);

    await new Promise(res=>{ const wait=()=>{ if(!isRecording) return res(true); requestAnimationFrame(wait); }; wait(); });
    return true;
  }catch{
    statusText.textContent="Mic permission or codec not supported.";
    endCall(); return false;
  }
}

/* ---------- BARGE-IN (interrupt during AI playback) ---------- */
const BARGE = { enable:true, rmsThresh:8, holdMs:120, cooldownMs:400 };
let bargeCtx=null, bargeSrc=null, bargeAnalyser=null, bargeRAF=null, bargeArmed=false, bargeSinceArm=0, bargeOnInterrupt=null;

async function ensureLiveMicForBargeIn(){
  try{
    if (globalStream && globalStream.getAudioTracks().some(t => t.readyState === "live")) {
      globalStream.getAudioTracks().forEach(t => { if (t.enabled === false) t.enabled = true; });
      return true;
    }
    ...
    updateMicTracks();
    return true;
  }catch(e){ warn("barge-in mic error", e); return false; }
}
function startBargeInMonitor(onInterrupt){
  ...
  bargeAnalyser.fftSize = 1024; bargeAnalyser.smoothingTimeConstant=0.8;
  bargeSrc.connect(bargeAnalyser);
  bargeArmed=false; bargeSinceArm=0; bargeOnInterrupt = onInterrupt;
  ...
  const loop=(ts)=>{
    ...
    if(!bargeArmed){
      bargeSinceArm += 16; // ~ frame guess
      if (bargeSinceArm >= BARGE.cooldownMs) bargeArmed = true;
    } else {
      if (rms > BARGE.rmsThresh) { hold += 16; if (hold >= BARGE.holdMs) { try{ onInterrupt?.(); }catch{} stopBargeInMonitor(); return; } }
      else hold = 0;
    }
...
function stopBargeInMonitor(){
  ...
  try{ bargeSrc?.disconnect(); bargeAnalyser?.disconnect(); }catch{}
  if (bargeCtx && bargeCtx.state!=="closed"){ try{ bargeCtx.close(); }catch{} }
...
async function playAIWithBargeIn(playableUrl, { aiBlob=null, aiBubbleEl=null } = {}){
  return new Promise(async resolve=>{
    ...
    // kick off live transcription while playing (if provided)
    if (!aiBubbleEl && inChatView) aiBubbleEl = appendMsg("ai","", { typing:true });
    if (aiBlob && N8N_TRANSCRIBE_URL) { try{ liveTranscribeBlob(aiBlob, aiBubbleEl).catch(()=>{}); }catch{} }

    // Ensure mic exists so we can hear the user during playback
    const okMic = await ensureLiveMicForBargeIn();

    let interrupted=false;
    const cleanup=()=>{ stopRing(); stopBargeInMonitor(); isPlayingAI=false; resolve({ interrupted }); };
    ...
    try{ await a.play(); }catch{} // ignore gesture errors (we already had a click)
    a.onended = ()=> cleanup();
  });
}

/* ---------- Voice path: upload → n8n → play AI ---------- */
const RECENT_USER_KEEP=12; let recentUserTurns=[];
async function uploadRecordingAndNotify(){
  ...
  if(combinedTranscript){
    ...
    if(recentUserTurns.length>RECENT_USER_KEEP) recentUserTurns.splice(0, recentUserTurns.length-RECENT_USER_KEEP);
  }
  ...
  if(!blob.size || !isCalling){ statusText.textContent="No audio captured."; return false; }
  ...
  if(!upRes.ok || !isCalling){ statusText.textContent=`Upload failed (${upRes.status}).`; return false; }
  ...
  }catch(e){ warn("webhook failed",e); statusText.textContent="AI processing failed."; return false; }

  if(!isCalling) return false;
  if(!aiPlayableUrl){ statusText.textContent="AI processing failed (no audio)."; return false; }

  // If JSON gave us text and chat is open, type it; else rely on live transcript webhook
  let aiBubble=null;
  if (inChatView){
  ...
  const { interrupted } = await playAIWithBargeIn(aiPlayableUrl, { aiBlob: (!aiTextFromJSON ? aiBlob : null), aiBubbleEl: aiBubble });
    ...
    while(true){
      ...
      for(const line of chunk.split(/\r?\n/)){
        ...
        try{
          ...
          if (evt.delta){ full += evt.delta; if(aiBubbleEl) aiBubbleEl.textContent=full; }
          if (evt.text && evt.done){ if(aiBubbleEl) aiBubbleEl.textContent=evt.text; return true; }
        }catch{}
      }
    }
    return !!full;
...
async function blobToFormData(blob, field="file", filename="audio.mp3"){
  const fd=new FormData(); fd.append(field, blob, filename); return fd;
}

/* ---------- Chat path ---------- */
async function sendChatToN8N(userText){
  const user_id=getUserIdForWebhook(); const device=getOrCreateDeviceId();
  ...
  try{
    ...
    if(ct.includes("application/json")){
      ...
      if(b64 && !url){
        const raw=b64.includes(",")?b64.split(",").pop():b64;
        const bytes=Uint8Array.from(atob(raw),c=>c.charCodeAt(0));
        b=new Blob([bytes],{type:data?.mime||"audio/mpeg"}); playableUrl=URL.createObjectURL(b); revoke=playableUrl;
    ...
    if(playableUrl){
      await playAIWithBargeIn(playableUrl, { aiBlob: (!aiText ? b : null), aiBubbleEl: aiBubble });
